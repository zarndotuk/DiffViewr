...
export function DiffValueCell({
  ...
  const color =
    kind === "missing"
      ...
      : kind === "extra"
        ...
        : kind === "changed"
          ? "bg-[color-mix(in_srgb,var(--accent)_12%,transparent)] border-[color-mix(in_srgb,var(--accent)_45%,transparent)]"
          : kind === "type_mismatch"
  ...
  return (
    <div className={`${base} ${color}`}>
      {label ? <span className="mr-2 text-[10px] uppercase text-[var(--muted)]">{label}</span> : null}
      {value}
    </div>
  );
}
